...
public class SearchAccount extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            // lay tu khoa de searchAccount/ managerAccount
            String acc = request.getParameter("keyword");

            //truy van data
            List<Account> listAccounts = new AcountDAO().search(acc);

            request.setAttribute("accounts", listAccounts);
            request.setAttribute("key",acc);
