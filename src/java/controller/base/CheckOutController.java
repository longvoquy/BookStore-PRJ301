...
public class CheckOutController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Order order = new Order(account.getUid(), totalPrice, note, shippingId);
        int orderId = new OrderDAO().createReturnId(order);

        // luu chi tiet don hang ban duoc
        ...
        for (Map.Entry<Integer, Cart> entry : carts.entrySet()) {
            ...
            int currentQuantity = product.getQuantityP();
            int newQuantity = currentQuantity - quantitySold;
            product.setQuantityP(newQuantity);
            productDAO.updateProductQuantity(product);
        }

        // xoa gio hang sau kho da thanh toan
        session.removeAttribute("carts");

        // thong bao da thuc hien mua hang thanh cong 
